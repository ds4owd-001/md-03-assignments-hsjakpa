...
5.  What data types are contained in the variables? (Reminder: <https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables>)
    ...
    | bill_length-mm,bill_depth_mm         | numerical           |
    | flipper_length_mm, body_mass_g, year | numerical -interger |
    |                                      |                     |
...
```{r}
penguins_bodymass_2900 <- penguins|>
...
```{r}
penguins_adelie_bill40mm <- penguins |>
  filter(species=="Adelie",
         bill_length_mm>40)
```
...
```{r}
penguins_notchinstrap <- penguins|>
...
```{r}
penguins_notgentoo_billD_15.5 <- penguins|>
...
```{r}
penguins_male_Dream_Biscoe_island <- penguins|>
  filter(sex =="male",
         island != "Torgersen")
```
...
```{r}
penguins_female_bill_length_35mm <- penguins|>
  filter(sex =="female",
         bill_length_mm <= 35)
```
...
```{r}
penguins_male_Dream_Torgerson_island <- penguins|>
  filter(sex =="female",
         island != "Biscoe")
```
...
```{r}
penguins_gentoos_4500g <- penguins|>
  filter(species =="Gentoo",
         body_mass_g>4500)
```
...
penguins <- penguins |>
  mutate(id = seq(1, length.out = nrow(penguins))) |>
...
penguins_groups <-  penguins|>
  ...
  summarise(bill_depth_mean= mean(bill_depth_mm, na.rm=T),
                                 bill_mean_length=mean(bill_length_mm,na.rm=T))
```
