...
```{r}
count(san_unimp_Gh|> group_by(year))
```
...
```{r}
ggplot(san_unimp_Gh,aes(residence,percent), fill=varname_long) +
  geom_col()+
  facet_wrap(year~.) +
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    *the bars are not arranged in eaither descending or ascending order*

2.  What would you want to change? *make them have a consistent orderly arrangement*

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    *so the data is not skewed, and thus we have true distribution of the data pertaining to the true nature of sanitation issues*
